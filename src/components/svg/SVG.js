...
function generateAriaLabel(title, description) {
    ...
    if (title || description) {
        ...
        if (description) attribute["aria-labelledby"] += `${generateUniqueId(description)}`
    }
    return attribute
}
...
const SVG = ({title, description, ...props}) => {
    return (
        <svg {...generateAriaLabel(title, description)} {...props}>
            {title && (
                <title id={generateUniqueId(title)}>{title}</title>
            )}
            {description && (
                <description id={generateUniqueId(description)}>{description}</description>
            )}
            {props.children}
        </svg>
    )
